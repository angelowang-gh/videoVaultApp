...
### Q: 使用 "Preview Folder (Browser)" 按钮后，文件夹没有添加到左侧目录？

**A**: 这是预期行为。由于浏览器安全限制，浏览器无法获取文件的完整路径，因此无法将文件夹添加到系统的扫描路径列表中。

**正确使用方法**：
1. 使用 "Preview Folder (Browser)" 预览文件夹内容，确认有视频文件
2. 记下文件夹名称
3. 使用 "Manual Path Input" 输入完整路径（如 `D:\Videos` 或 `/home/user/videos`）
4. 点击 "Add" 按钮永久添加

**为什么这样设计？**
- 浏览器安全沙箱不允许访问完整文件系统路径
- 后端服务器需要绝对路径才能扫描视频文件
- 这是所有 Web 应用的共同限制
